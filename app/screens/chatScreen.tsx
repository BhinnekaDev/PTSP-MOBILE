...
export default function Chat() {
  ...
  return (
    <View className="flex-1">
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 140, paddingHorizontal: 10 }} showsVerticalScrollIndicator={false}>
        <TouchableOpacity onPress={toggleMessages} activeOpacity={0.6} className="flex-row gap-2 items-center pt-6 pb-2 px-4">
        ...
        {showMessages && (
          <Animated.View entering={FadeInUp.duration(400)}>
            {stations.map((station, idx) => (
              <TouchableOpacity key={idx} className="flex-row gap-4 items-center p-2 mt-2 bg-white rounded-xl shadow" activeOpacity={0.5} onPress={() => router.push("/screens/roomChatScreen")}>
                <View className="bg-gray-300 rounded-full p-4">{station.icon}</View>
                <View className="flex-1">
                  <Text style={{ fontFamily: "LexBold" }} className="text-lg">
                    {station.name}
                  </Text>
                  <Text style={{ fontFamily: "LexRegular" }}>{"Lorem ipsum dolor sit amet consectetur...".slice(0, 30) + "..."}</Text>
                </View>
                <View className="flex-col justify-center items-center">
                  <Text style={{ fontFamily: "LexRegular" }} className="text-sm">
                    {station.date}
                  </Text>
                  <Text className="bg-red-500 rounded-full py-px px-2 text-sm text-white" style={{ fontFamily: "LexRegular" }}>
                    {station.unread}
                  </Text>
                </View>
              </TouchableOpacity>
