...
export const useGoogleLogin = () => {
  ...
  const checkUserRegistration = async (uid: string) => {
    ...
    const [peroranganDoc, perusahaanDoc] = await Promise.all([peroranganRef.get(), perusahaanRef.get()]);
  ...
  const signIn = async () => {
    try {
      ...
      if (!idToken) throw new Error("ID Token tidak ditemukan setelah login Google.");
      ...
      if (registrationStatus === "perorangan" || registrationStatus === "perusahaan") {
        console.log(`✅ User terdaftar sebagai ${registrationStatus} → arahkan ke Home`);
        router.replace("/(tabs)/home");
      } else {
        console.log("🆕 User belum terdaftar → arahkan ke pemilihan form register");
